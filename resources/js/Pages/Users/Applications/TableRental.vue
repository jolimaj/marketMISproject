    ...
    <AppLayout title="Stalls">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                :href="`/my-rentals/table-spaces/create`" class="bg-btn-gradient text-white font-bold uppercase px-5 py-2 rounded focus:outline-none shadow hover:bg-primary-700 transition-colors">Rent New</Link>
            </div>
            <BaseTable :headers="headers" :data="tableRentals?.data" :class="'center text-center sticky right-0 bg-white z-10'">
                <template #row="{ data }">
                    <tr v-for="(bsn, i) in data" :key="tableRentals.id" class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 capitalize">
                            <Link class="flex items-center pr-2" :href="`/my-rentals/table-spaces/${bsn.id}`" tabindex="-1">
...
const formData = useForm({ 
  ...
  reference_number: '',
  amount: 0,
  table_rental_id: null,
});

watch(
  form,
  throttle(() => {
    router.get(`/my-rentals/table-spaces`, pickBy(form), { preserveState: true })
  }, 150),
  { deep: true }
);
...
function payNow(data) {
    ...
    formData.table_rental_id = Number(data?.id);
...
const generateInvoicePDF = (data) => {
  ...
  doc.text(`Table Renter Name: ${data.applicant_name}`, 14, 49);
  ...
  autoTable(doc, {
    ...
    body: [["Table Rental Payment", `${formatAmount(data.amount)}`]],
    theme: "grid",
  });
