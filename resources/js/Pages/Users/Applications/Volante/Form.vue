    ...
    <AppLayout title="Stall Types">
        <template #header>
         ...
         <h1 class="mb-8 text-3xl font-bold">
          ...
          {{checkIfEdit() || checkIfReupload() ? `${props?.volanteRental?.name}` : 'Create New'}}
        </h1>
          ...
          <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
            ...
            <form @submit.prevent="checkIfEdit() || checkIfReupload() ? update() : nextButton()">
              ...
              <div v-if="step === 2 || checkIfReupload()" class="p-6 space-y-6">
                <DocumentUpload
                    :requiredDocs="requirements?.volante_new"
                ...
                <div v-if="step === 4" class="p-6 space-y-6">
                  <h2 class="text-lg font-bold text-secondary mb-2">
                    ...
                    <div>
                        ...
                        <p class="text-gray-600">Slot Name: {{ selectedStall?.name }}</p>
                    ...
                    <div class="bg-gray-50 rounded-lg p-4 space-y-2 text-sm">
                      ...
                      <p class="font-bold text-gray-600">
                          Calculated Monthly Payment: <span class="text-green-600 font-semibold">{{ formatAmount(total_payments?.total) }}</span>
                      </p>
                    </div>
                </div>
              </div>
              ...
              <div class="flex items-center px-8 py-4 bg-gray-50 border-t border-gray-100">
                <Link :href="checkIfEdit() ? `/my-rentals/market-volante/${volanteRental?.id}` : '/my-rentals/market-volante'" class="text-red-600 hover:underline" tabindex="-1">Cancel</Link>
...
import { formatAmount, formatDateShort, fullName, isEditPage, isReuploadPage, mapTablesToGeoJSON } from '@/data/helper';
...
const form = useForm({ 
  type: checkIfEdit() || checkIfReupload() ?  props?.volanteRental?.permits?.permit_type : 1,
  business_name: checkIfEdit() || checkIfReupload() ?  props?.volanteRental?.name : '',
  stall_id: checkIfEdit() || checkIfReupload() ? props?.volanteRental?.stalls?.id : '',
  requirements: [],
  acknowledgeContract: checkIfEdit() || checkIfReupload() ? props?.volanteRental?.acknowledgeContract : '',
  reference_number: checkIfEdit() || checkIfReupload() ? props?.volanteRental?.paymentRecord?.reference_number : '',
  attachment_signature: [],
  total_payment: checkIfEdit() || checkIfReupload() ? props?.volanteRental?.paymentRecord?.total_payments : 0,
  fees: checkIfEdit() || checkIfReupload() ? Array.isArray(props?.volanteRental?.fees_additional)
  ? props?.volanteRental?.fees_additional.map(Number)
  : [] : [],
  step,
  bulb: checkIfEdit() || checkIfReupload() ? props?.volanteRental?.bulb : null,
  stall_rental_id: checkIfEdit() || checkIfReupload() ? props?.volanteRental?.id : null,
  ...
  if(isGenerating.value) {
    form.post(route("applications.volantes.store"), {
      ...
      onSuccess: async () => {
        ...
        router.visit(route("applications.volantes.index"), {
          preserveScroll: true,
          preserveState: false,
        });
      },
...
const renewals = async () => {
    ...
    Inertia.post(route('applications.volantes.update', props?.volanteRental?.id), fd, {
      preserveScroll: true,
...
const nextButton = () => {
  if (step.value === 1) {
    ...
    if (checkIfEdit()) {
      ...
      const totalRequired = props?.requirements?.volante_new?.length || 0;
...
onMounted(() => {
  if (checkIfEdit()) {
    onStallSelect(props?.volanteRental?.stalls);
  }
});
